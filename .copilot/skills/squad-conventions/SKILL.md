...
Tests use `node:test` and `node:assert/strict` — no test frameworks. Run with `pnpm test`. Test files live in `test/`. The test command is `node --test test/`.
...
- `templates/` — Source templates shipped with the pnpm package
...
## Anti-Patterns
- **Adding pnpm dependencies** — Squad is zero-dep. Use Node.js built-ins only.
