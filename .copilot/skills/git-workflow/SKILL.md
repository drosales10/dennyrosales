...
|--------|---------|-----------|
| `main` | Released, tagged, in-pnpm code only | `pnpm publish` on tag |
| `dev` | Integration branch — all feature work lands here | `pnpm publish --tag preview` on merge |
| `insiders` | Early-access channel — synced from dev | `pnpm publish --tag insiders` on sync |
...
```bash
# Node.js / pnpm
cd ../squad-sdk && pnpm link
cd ../squad-pr && pnpm link squad-sdk
...
**Important:** Remove local links before committing. `pnpm link` and `go replace` are dev-only — CI must use published packages or PR-specific refs.
