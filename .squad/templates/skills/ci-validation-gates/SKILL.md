...
Every publish workflow MUST validate version format before `pnpm publish`. 4-part versions (e.g., 0.8.21.4) are NOT valid semver — pnpm mangles them.
...
- name: Validate semver
  run: |
    ...
    VERSION="${VERSION#v}"
    if ! pnpm dlx semver "$VERSION" > /dev/null 2>&1; then
...
### PNPM Token Type Verification
PNPM_TOKEN MUST be an Automation token, not a User token with 2FA:
- User tokens require OTP — CI can't provide it → EOTP error
- Create Automation tokens at js.com → Settings → Access Tokens → Automation
- Verify before first publish in any workflow

### Retry Logic for pnpm Registry Propagation
pnpm registry uses eventual consistency. After `pnpm publish` succeeds, the package may not be immediately queryable.
...
|---|---------------|-----------|------------|
| 1 | 4-part version published, pnpm mangled it | No semver validation gate | `pnpm dlx semver` check before every publish |
...
- ❌ Assuming pnpm publish is instantly queryable
