...
This is the **definitive release runbook** for Squad. Born from the v0.8.22 release disaster (4-part semver mangled by pnpm, draft release never triggered publish, wrong PNPM_TOKEN type, 6+ hours of broken `latest` dist-tag).
...
**Rule:** Only 3-part semver (major.minor.patch) or prerelease (major.minor.patch-tag.N) are valid. 4-part versions (0.8.21.4) are NOT valid semver and pnpm will mangle them.
...
### 2. PNPM_TOKEN Verification

**Rule:** PNPM_TOKEN must be an **Automation token** (no 2FA required). User tokens with 2FA will fail in CI with EOTP errors.

```bash
# Check token type (requires pnpm CLI authenticated)
pnpm token list
```
...
4. Copy token and save as GitHub secret: `PNPM_TOKEN`
...
pnpm version $VERSION --workspaces --include-workspace-root --no-git-tag-version
...
2. Verify step runs with retry loop (up to 5 attempts, 15s interval) to confirm SDK on pnpm registry
3. `publish-cli` job runs → publishes `@bradygaster/squad-cli`
4. Verify step runs with retry loop to confirm CLI on pnpm registry

**If workflow fails:** Check the logs. Common issues:
- EOTP error = wrong PNPM_TOKEN type (use Automation token)
- Verify step timeout = pnpm propagation delay (retry loop should handle this, but propagation can take up to 2 minutes in rare cases)
- Version mismatch = package.json version doesn't match tag

**Checkpoint:** Both jobs succeeded. Workflow shows green checkmarks.

### Step 5: Verify pnpm Publication

Manually verify both packages are on pnpm with correct `latest` dist-tag.

```bash
# Check SDK
pnpm view @bradygaster/squad-sdk version
# Output: 0.8.22

pnpm dist-tag ls @bradygaster/squad-sdk
# Output should show: latest: 0.8.22

# Check CLI
pnpm view @bradygaster/squad-cli version
# Output: 0.8.22

pnpm dist-tag ls @bradygaster/squad-cli
# Output should show: latest: 0.8.22
```

**If versions don't match:** Something went wrong. Check workflow logs. DO NOT proceed with GitHub Release announcement until pnpm is correct.

**Checkpoint:** Both packages show correct version. `latest` dist-tags point to the new version.

### Step 6: Test Installation

Verify packages can be installed from pnpm (real-world smoke test).
...
pnpm init --bare
pnpm install @bradygaster/squad-sdk
node -p "require('@bradygaster/squad-sdk/package.json').version"
# Output: 0.8.22

# Test CLI installation
pnpm install -g @bradygaster/squad-cli
squad --version
# Output: 0.8.22
...
**If installation fails:** pnpm registry issue or package metadata corruption. DO NOT announce release until this works.
...
# Update all 3 package.json files
pnpm version $NEXT_VERSION --workspaces --include-workspace-root --no-git-tag-version
...
### 1. Unpublish from pnpm (Nuclear Option)

**WARNING:** pnpm unpublish is time-limited (24 hours) and leaves the version slot burned. Only use if version is critically broken.

```bash
# Unpublish (requires pnpm owner privileges)
pnpm unpublish @bradygaster/squad-sdk@0.8.22
pnpm unpublish @bradygaster/squad-cli@0.8.22
```

### 2. Deprecate on pnpm (Preferred)

**Preferred approach:** Mark version as deprecated, publish a hotfix.

```bash
# Deprecate broken version
pnpm deprecate @bradygaster/squad-sdk@0.8.22 "Broken release, use 0.8.22.1 instead"
pnpm deprecate @bradygaster/squad-cli@0.8.22 "Broken release, use 0.8.22.1 instead"
...
**Checkpoint:** Tag and release deleted. main branch reverted. pnpm packages deprecated or unpublished.
...
**Root cause:** PNPM_TOKEN is a User token with 2FA enabled. CI can't provide OTP.  
**Fix:** Replace PNPM_TOKEN with an Automation token (no 2FA). See "PNPM_TOKEN Verification" above.

### Verify Step 404 (pnpm Propagation Delay)

**Symptom:** Verify step fails with 404 even though publish succeeded.  
**Root cause:** pnpm registry propagation delay (5-30 seconds).  
...
**Fix:** Ensure all 3 package.json files were updated in Step 1. Re-run `pnpm version` if needed.

### 4-Part Version Mangled by pnpm

**Symptom:** Published version on pnpm doesn't match package.json (e.g., 0.8.21.4 became 0.8.2-1.4).  
**Root cause:** 4-part versions are NOT valid semver. pnpm's parser misinterprets them.  
...
- [ ] PNPM_TOKEN is an Automation token (no 2FA): `pnpm token list` shows `read-write` without OTP requirement
...
- [ ] SDK on pnpm: `pnpm view @bradygaster/squad-sdk version` returns correct version
- [ ] CLI on pnpm: `pnpm view @bradygaster/squad-cli version` returns correct version
- [ ] `latest` tags correct: `pnpm dist-tag ls @bradygaster/squad-sdk` shows `latest: VERSION`
- [ ] Packages install: `pnpm install @bradygaster/squad-cli` succeeds
...
2. Semver validation is mandatory — 4-part versions break pnpm
3. PNPM_TOKEN type matters — User tokens with 2FA fail in CI
4. Draft releases are a footgun — they don't trigger automation
5. Retry logic is essential — pnpm propagation takes time
